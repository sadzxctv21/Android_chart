...
public class line_chart extends View implements Runnable {
    Context context;
    public line_chart(Context context) {
        super(context);
    ...
    public line_chart(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context=context;
        init();
        new Thread(this).start();
    ...
    private void labelText(Canvas canvas){
        ...
        canvas.drawText("●",300,chartH+spaceY*1.8f,mLine2);
        canvas.drawText(LabelText2,350,chartH+spaceY*1.8f,mLineText);
    }
    ...
    private void displayValue(Canvas canvas){
        int oldX1=0,oldY1=0;
        int oldX2=0,oldY2=0;
        for (int a=0;a<ValueMax;a++){
            int newX1=spaceX+(a)*chartW/ValueMax;
            int newY1=chartH-(((Value1[a]-Min_Y)*chartH/(Max_Y-Min_Y))-spaceY);
            int newX2=newX1;
            int newY2=chartH-(((Value2[a]-Min_Y)*chartH/(Max_Y-Min_Y))-spaceY);
            if (Circle==true){
                canvas.drawCircle(newX1,newY1,5,mLine1);
                canvas.drawCircle(newX2,newY2,5,mLine2);
            }

            if (a!=0 &Value1[a]!=0&Value2[a]!=0){
                canvas.drawLine(oldX1,oldY1,newX1,newY1,mLine1);
                canvas.drawLine(oldX2,oldY2,newX2,newY2,mLine2);
            }
            oldX1=newX1;
            oldY1=newY1;
            oldX2=newX2;
